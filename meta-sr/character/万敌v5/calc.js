export const details = [{
  ...
  title: '[E]弑王成王主目标伤害',
  params: { XC: true , A: false},
  dmg: ({ talent, attr }, { basic }) => basic(talent.e2['弑王成王目标伤害'] * attr.hp, 'e')
},{
  title: '[E]弑王成王相邻目标伤害',
  params: { XC: true , A: false},
  dmg: ({ talent, attr }, { basic }) => basic(talent.e2['弑王成王相邻目标伤害'] * attr.hp, 'e')
},{
  title: '[E]弑神登神主目标伤害',
  dmgKey: 'AE',
  check: ({ cons }) => cons < 1,
  params: { XC: true , A: false},
  dmg: ({ talent, attr }, { basic }) => basic(talent.e2['弑神登神目标伤害'] * attr.hp * 1.8, 'e')
},{
  title: '[E]弑神登神相邻目标伤害',
  check: ({ cons }) => cons < 1,
  params: { XC: true , A: false},
  dmg: ({ talent, attr }, { basic }) => basic(talent.e2['弑神登神相邻目标伤害'] * attr.hp * 1.8, 'e')
},{
  title: '[E]弑神登神主目标伤害',
  dmgKey: 'AE',
  check: ({ cons }) => cons > 0,
  params: { XC: true , A: false},
  dmg: ({ talent, attr }, { basic }) => basic(talent.e2['弑神登神目标伤害'] * 1.3 * attr.hp * 1.8, 'e')
},{
  title: '[E]弑神登神其他全体目标伤害',
  check: ({ cons }) => cons > 0,
  params: { XC: true , A: false},
  dmg: ({ talent, attr }, { basic }) => basic(talent.e2['弑神登神目标伤害'] * attr.hp * 1.8, 'e')
},{
...
export const buffs = [{
  check: ({ params,cons}) => params.XC === true,
  ...
  data: {
    cpct: ({ calc, attr }) => Math.min(114514, (calc(attr.hp) - 4000 ) / 100 * 1.2)
 }
}, {
  check: ({ params }) => params.XC === true,
  title: '血仇：无视敌方目标15%的防御力',
  cons: 2,
  data: {
    ignore: 15
 }
