export const details = [{
  ...
  title: '[默认12韧性目标]击破伤害',
  dmg: ({ calc, attr,talent}, { reaction }) => {
    return {
      avg: reaction('imaginaryBreak').avg * (12 + 2) / 4
    }
  }
},{
  check: ({ calc, attr,cons }) => cons < 6,
  ...
  dmg: ({ calc, attr,talent}, { reaction }) => {
    return {
      avg: (reaction('imaginaryBreak').avg * (12 + 2) / 4) * talent.t['击破伤害']
    }
  }
},{
  ...
  dmg: ({ calc, attr,talent}, { reaction }) => {
    return {
      avg: (reaction('imaginaryBreak').avg * (12 + 2) / 4) * (talent.t['击破伤害'] + 1.2)
    }
  }
},{
  title: '[结印状态]一段主目标超击破伤害',
  dmg: ({ talent }, { reaction }) => {
    return {
      avg: reaction('superBreak').avg * 1 * 1 * 0.6 / 0.9
    }
  }
},{
  title: '[结印状态]一段相邻目标超击破伤害',
  dmg: ({ talent }, { reaction }) => {
    return {
      avg: reaction('superBreak').avg * 0.5 * 1 * 0.6 / 0.9
    }
  }
},{
  title: '[结印状态]二段主目标超击破伤害',
  dmg: ({ talent }, { reaction }) => {
    return {
      avg: reaction('superBreak').avg * 1 * 1 * 0.6 / 0.9
    }
