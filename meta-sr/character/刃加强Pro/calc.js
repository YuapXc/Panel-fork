export const details = [{
  ...
  dmg: ({ talent, attr, calc }, { basic }) => {
    const td1 = talent.a['技能伤害']
    const hp = calc(attr.hp)
    return basic(td1 * hp, 'a')
  }
}, {
  title: '强化普攻(单体)',
  ...
  params: { ebuff: true },
  dmg: ({ talent, attr, calc }, { basic }) => {
    const td1 = talent.t['生命倍率']
    const hp = calc(attr.hp)
    return basic(td1 * hp, 't')
  }
}, {
  title: '终结技伤害(单体)',
  params: { ebuff: true },
  dmg: ({ talent, attr, calc }, { basic }) => {
    const td1 = talent.q['生命倍率']
    const td2 = talent.q['已损失生命值倍率']
    const hp = calc(attr.hp)
    return basic(td1 * hp + td2 * hp * 0.9, 'q')
  }
}, {
  title: '终结技伤害(扩散)',
  params: { ebuff: true },
  dmg: ({ talent, attr, calc }, { basic }) => {
    const td1 = talent.q['生命倍率'] + talent.q['生命倍率·相邻目标'] * 2
    const td2 = talent.q['已损失生命值倍率'] + talent.q['已损失生命值倍率·相邻目标'] * 2
    const hp = calc(attr.hp)
    return basic(td1 * hp + td2 * hp * 0.9, 'q')
  }
}]
