export default function (staticIdx, keyIdx) {
  return {
    ...
    无边曼舞: [
      staticIdx(1, 'cpct'),
      keyIdx('对处于防御降低或减速状态下的敌人造成的暴击伤害提高[cdmg]%', 'cdmg', 2)
    ],
    晚安与睡颜: [
      (tables) => {
        return {
    ...
    那无数个春天: [
      staticIdx(1, 'effPct'),
      keyIdx('敌方目标受到的伤害提高[cpct]%', 'dmg', 2),
      keyIdx('敌方目标受到的伤害额外提高[dmg]%', 'dmg', 3)
    ],
    行于流逝的岸: [
      staticIdx(1, 'cdmg'),
      keyIdx('陷入【泡影】状态的目标造成的伤害提高[dmg]%', 'dmg', 2),
      keyIdx('终结技造成的伤害额外提高[qdmg]%', 'qdmg', 3)
    ],
    只需等待: [
      staticIdx(1, 'dmg'),
      (tables) => {
    ...
    长路终有归途: [
      staticIdx(1, 'stance'),
      keyIdx('敌方受到装备者造成的击破伤害提高[breakEnemydmg]%', 'breakEnemydmg', 3)
    ],
    好戏开演: [
      (tables) => {
        return {
