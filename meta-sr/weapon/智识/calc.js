export default function (staticIdx, keyIdx) {
  return {
    ...
    不息的演算: [
      staticIdx(1, 'atkPct'),
      keyIdx('攻击力额外提高[atkPct]%', 'atkPct', 3),
      keyIdx('自身速度提高[speddPct]%', 'speddPct', 4)
    ],
    偏偏希望无价: [
      staticIdx(1, 'cpct'),
      keyIdx('追加攻击造成的伤害提高[tdmg]%', 'tdmg', 3),
      keyIdx('终结技造成的伤害无视目标[qignore]%防御', 'qignore', 4),
      keyIdx('追加攻击造成的伤害无视目标[tignore]%防御', 'tignore', 4)
    ],
    今日亦是和平的一日: (tables) => {
      return {
        title: '根据能量上限提高数据',
