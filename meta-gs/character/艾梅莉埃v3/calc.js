...
export const buffs = [{
  ...
  title: '精馏：对燃烧状态下的敌人造成的伤害提升[dmg]%',
  data: {
    dmg: ({ attr }) => Math.min(36, attr.atk/1000*15)
  }
},{
  check: ({ params , cons }) => ( cons > 0 ),
  title: '1命：撷萃调香与固有天赋「余薰」的清露香氛造成的伤害提升20%',
  data: {
    edmg: 20,
    tdmg: 20
  }
  ...
  data: {
    aPlus: ({ calc, attr }) => calc(attr.atk) * 3,
    ePlus: ({ calc, attr }) => calc(attr.atk) * 3,
    qPlus: ({ calc, attr }) => calc(attr.atk) * 3
  }
},
